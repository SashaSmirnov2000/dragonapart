...
export async function POST(req: Request) {
  try {
    ...
    console.log("Входящий запрос:", body); // Увидим в логах Vercel
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.error("Ошибка: Переменные Supabase не найдены!");
    ...
    if (body.apartment_id && body.user_id) {
      // Это сработает, когда ты нажмешь кнопку на сайте
      await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    console.error("Критическая ошибка Webhook:", error.message);
    return NextResponse.json({ ok: true }); // Возвращаем ok, чтобы ТГ не спамил
  }
}